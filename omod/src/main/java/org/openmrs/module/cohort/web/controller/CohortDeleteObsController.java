 ...
 */
@Controller
public class CohortDeleteObsController {

    protected final Log log = LogFactory.getLog(getClass());

    @RequestMapping(value = "/module/cohort/deleteobs.form", method = RequestMethod.GET)
    public void manage(HttpSession httpSession, HttpServletRequest request, ModelMap model, @RequestParam(required = false, value = "encounterId") Integer id, @ModelAttribute("cohortencounter") CohortEncounter cencounter) {
        model.addAttribute("cohortobs", new CohortObs());
        CohortService departmentService = Context.getService(CohortService.class);
        if ("delete".equals(request.getParameter("delete"))) {
            List<CohortObs> list1 = departmentService.findCohortObs();
            for (int i = 0; i < list1.size(); i++) {
                CohortObs c = list1.get(i);
                if (c.getObsId().equals(id)) {
                    try {
                        departmentService.purgeCohortObs(c);
